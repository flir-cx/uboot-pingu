...
int do_bq27(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (!strcmp(argv[1], "status")) {
		ret = bq27_read(BQ_CONTROL_STATUS, &value);
	} else if (!strcmp(argv[1], "type")) {
		ret = bq27_read(BQ_DEVICE_TYPE, &value);
	} else if (!strcmp(argv[1], "fwver")) {
		ret = bq27_read(BQ_FW_VERSION, &value);
	} else if (!strcmp(argv[1], "dfver")) {
		ret = bq27_read(BQ_DF_VERSION, &value);
	} else if (!strcmp(argv[1], "chemid")) {
		ret = bq27_read(BQ_CHEM_ID, &value);
	} else if (!strcmp(argv[1], "volt")) {
		ret = bq27_read(BQ_VOLTAGE, &value);
	} else if (!strcmp(argv[1], "soc")) {
		ret = bq27_read(BQ_STATE_OF_CHARGE, &value);
	} else if (!strcmp(argv[1], "dcap")) {
		ret = bq27_read(BQ_DESIGN_CAPACITY, &value);
	} else {
